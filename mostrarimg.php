    ...
    <center>
        ...
        if (!$conexion) {
            ...
            $ejecutarConsulta = mysqli_query(
                $conexion,
                "SELECT cadenaImg as cadena FROM zpruebaimagenes where idprueba=14"
            )
        ...
        ?>
        <script>
            var data = <?php echo json_encode($fila['cadena']); ?>;
            ...
            // var objectURL = URL.createObjectURL(new Blob(binaryData, {type: "image"}));
            // imagen.src=objectURL;
        </script>
